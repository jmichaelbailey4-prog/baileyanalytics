...
def build_alert_body(result, repo, workflow, runs_url, marker):
    ...
    return (
        f"{marker}\n\n"
        f"**The `{workflow}` pipeline may have stopped publishing.**\n\n"
...
def fetch_runs(repo, workflow, token, per_page=30):
    ...
    return _request("GET", url, token).get("workflow_runs", [])
...
def main(argv=None):
    ...
    threshold = float(os.environ.get("FRESHNESS_THRESHOLD_HOURS")
