...
- **Empty state:** an inviting panel — "You haven't saved any lenses yet" + a one-line what/why + a button to **Dashboards**, so a visitor who opens Favorites with nothing saved is guided rather than stranded.
...
- **`/manifest.webmanifest`** (static): `name` "Bailey Analytics", `short_name` "Bailey", `description`, `start_url` `/` (home, D3), `scope` `/`, `display` `standalone`, `background_color`/`theme_color` `#0A0E14`, `icons` (192/512 + maskable), `orientation` "any".
